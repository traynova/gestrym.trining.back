...
src/training/interfaces/http/handlers/
  TrainingPlanHandler.go             ← Added `/adapt` and `/internal/ai` endpoints
...
### Internal Endpoints (`/gestrym-training/internal/training-plans`)

| Method | Path | Security | Description |
|---|---|---|---|
| POST | `/ai` | X-API-Key | Stores plan generated by external AI-Service |
...
- Future AI plan generation should create a `TrainingPlan` with `IsTemplate = false` and set `CreatedBy = 0` (System).
- **AI-Service Integration**: Use `POST /internal/training-plans/ai` to receive fully structured plans from the AI brain.
