...
import (
	...
	"gestrym-training/src/common/routes"
...
func setupEnvironment(isLocalEnv bool) error {
	if isLocalEnv {
		logger.Info("[GESTRYM_TRAINING] server started in development mode")
		...
		logger.Info("[GESTRYM_TRAINING] server started in production mode")
...
func initServer() {
	address := viper.GetString("GESTRYM_TRAINNER_SERVER_ADDRESS")
	ginMode := viper.GetString("GIN_MODE")
	if address == "" {
		logger.Fatal("Server address env 'GESTRYM_TRAINNER_SERVER_ADDRESS' not set")
	}
	if ginMode == "" {
	...
	logger.Info("[GESTRYM_TRAINING] Start server on -> %v", address)
